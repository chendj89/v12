...
class Particle {
  ...
  public diff: number;
  constructor(shape: string, coor: { x: number; y: number }, offset: number) {
    this.shape = shape;

    this.coor = coor;
    this.position = coor.y;

    this.offset = offset;
    this.steps = oHeight / 2;

    this.rotation = Math.floor(Math.random() * 5 + 1);
    this.scale = 0.5 + Math.random() * 0.5;
    this.siner = oHeight / 2;
    this.diff = (oHeight * Math.random()) / 2;
    this.rect = this.init();
  }

  init() {
    let div: HTMLElement | null = document.createElement('div');
    div.innerHTML = this.shape;
    this.el = div.childNodes[0] as HTMLElement;
    const target = document.getElementById('el');
    target?.appendChild(this.el);
    div = null;
    const fills = ['#8491A3', '#2DA94F', '#FDBD00', '#ED412D'];
    this.el.style.width = '30px';
    this.el.style.height = '30px';
    this.el.style.transform = `translate(${this.coor.x}px,${this.coor.y}px)`;
    this.el.style.fill = fills[random(0, fills.length - 1)];
  ...
  render() {
    ...
    const left = this.coor.x + Math.sin((this.diff + this.position / oHeight) * Math.PI) * 100;
    this.el.style.transform = `translateY(${top}px) translateX(${left}px) scale(${this.scale}) rotate(${this.rect.height + this.rotation * this.position}deg)`;
    if (this.position < -this.rect.height) {
      this.destory();
      return false;
    } else {
      return true;
...
const data: string[] = [point, rhombus, pentahedron, circle, x];
// setInterval(() => {
